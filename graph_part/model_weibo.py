...
from utils import my_weight_init, draw_picture
from layer import VLTransformer
# from model import ClippyAdam
...
class NeuralNetwork(nn.Module):
    ...
    def fit(self, X_train_tid, X_train, y_train,
        ...
        # self.optimizer = ClippyAdam(self.parameters(), lr=2e-3, clip_accumulator_update=True)
        ...
        for epoch in range(self.config['epochs']):
            ...
            for i, data in enumerate(dataloader):
                ...
                acc_list.append(acc_.cpu().numpy())
            valid_acc= self.evaluate(X_dev_tid, X_dev, y_dev)
            valid_acc_list.append(valid_acc)
            avg_acc_list.append(np.mean(acc_list))
            avg_loss_list.append(np.mean(loss_list))
        return avg_loss_list, avg_acc_list, valid_acc_list, self.config['epochs']

    def evaluate(self, X_dev_tid, X_dev, y_dev):
        correct = self.predict(X_dev_tid, X_dev)
...
class MFAN(NeuralNetwork):
    ...
    def forward(self, X_tid, X_text):
        ...
        fusion_input = torch.concat([self_att_t, self_att_i, self_att_g], dim=-1).squeeze(dim=1)
...
def train_and_test(model):
    ...
    loss_list, acc_list, valid_acc_list, epoch_num = nn.fit(X_train_tid, X_train, y_train,
                                                                             X_dev_tid, X_dev, y_dev)
    x = [i + 1 for i in range(epoch_num)]
    x_label = "Epoch"
    y1_label = "Acc"
    y2_label = "Loss"
    draw_picture(x, loss_list, x_label, y2_label,title="Loss Weibo(Train)")
    draw_picture(x, acc_list, x_label, y1_label, x2=x, y2=valid_acc_list, title="Acc Weibo(Train)")
