...
class MFAN(NeuralNetwork):
    def __init__(self, config, adj, original_adj):
        ...
        self.fc3 = nn.Linear(in_features=900, out_features=512)
    ...
    def forward(self, X_tid, X_text):
        ...
        # MLP
        a1 = self.relu(self.dropout(self.fc3(fusion_input)))
        a1 = self.relu(self.fc4(a1))
        hidden = self.relu(self.fc1(a1))
        prob = F.log_softmax(hidden)
...
def train_and_test(model):
    ...
    config['save_path'] = os.path.join(res_dir, args.thread_name + '_' + 'config' + args.config_name.split(".")[
        0] + '_best_model_weights_' + model_suffix+'_wo_fusion')
    ...
    nn.load_state_dict(torch.load(
        "./exp_result/weibo2/exp_description/best_model_in_each_config/Thread-1_configsingle3_best_model_weights_mfan_wo_fusion"))
    # 当前最佳模型
    # nn.load_state_dict(torch.load(
    #     "./exp_result/weibo2/exp_description/best_model_in_each_config/best_model_weibo"))
