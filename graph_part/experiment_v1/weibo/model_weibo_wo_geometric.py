...
class NeuralNetwork(nn.Module):
    ...
    def mfan(self, x_tid, x_text, y, loss, i, total, params, pgd_word):
        ...
        loss_defense = loss_classification + loss_dis + 0.01 * loss_constrative
...
def train_and_test(model):
    ...
    config['save_path'] = os.path.join(res_dir, args.thread_name + '_' + 'config' + args.config_name.split(".")[
        0] + '_best_model_weights_' + model_suffix + '_wo_geometric')
    ...
    nn.load_state_dict(torch.load(
        "./exp_result/weibo2/exp_description/best_model_in_each_config/Thread-1_configsingle3_best_model_weights_mfan_wo_geometric"))
    # 当前最佳模型
    # nn.load_state_dict(torch.load(
