...
class MFAN(NeuralNetwork):
    def __init__(self, config, adj, original_adj):
        ...
        fusion_dim = config['fusion_dim_2']
        ...
        self.afn = hard_fc(300, 300)
        self.relu = nn.ReLU()
        self.tanh = nn.Tanh()
        self.fc1 = nn.Linear(in_features=600, out_features=300)
    ...
    def forward(self, X_tid, X_text):
        ...
        text_feature = self.fc1(conv_feature)
        iembedding = self.fc2(iembedding)

        # text_feature->[bach_size,300],graph_feature->[bach_size,300],iembedding->[batch_size,300]
        fusion_input = torch.concat([text_feature, iembedding], dim=-1)
        prob, hidden, attn = self.model_fusion(fusion_input)

        # 特征对齐
        align_text = self.afn(text_feature)
        align_rembedding = self.afn(iembedding)
