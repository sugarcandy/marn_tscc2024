...
class NeuralNetwork(nn.Module):
    ...
    def mfan(self, x_tid, x_text, y, loss, i, total, params, pgd_word):
        ...
        # 这里调用mfan的forward
        logit_original, dist_og, features = self.forward(x_tid, x_text)
        loss_classification = loss(logit_original, y)
        # 分类损失
        loss_mse = nn.MSELoss()
        # 模态对齐损失
        loss_dis_1 = loss_mse(dist_og[0], dist_og[1])
        loss_dis_2 = loss_mse(dist_og[0], dist_og[2])
        loss_dis_3 = loss_mse(dist_og[1], dist_og[2])
        loss_dis = (loss_dis_1 + loss_dis_2 + loss_dis_3) / 3
    ...
    def predict(self, X_test_tid, X_test):
        ...
        for i, data in enumerate(dataloader):
            with torch.no_grad():
                ...
                logits, dist, _ = self.forward(batch_x_tid, batch_x_text)
...
class MFAN(NeuralNetwork):
    def __init__(self, config, adj, original_adj):
        ...
        self.aft = hard_fc(d_in=900, d_hid=900, DroPout=self.config['dropout'])
    ...
    def forward(self, X_tid, X_text):
        ...
        # resnet50获取图像嵌入
        image_feature = self.image_embedding.forward(X_tid)
        ...
        for _, (Conv, max_pooling) in enumerate(zip(self.convs, self.max_poolings)):
            ...
            text_feature.append(pool)

        text_feature = torch.cat(text_feature, dim=1)
        # bsz = len(text_feature)

        graph_feature = rembedding

        # text_feature->[bach_size,300],graph_feature->[bach_size,300],iembedding->[batch_size,300]
        fusion_input = torch.concat([text_feature, image_feature, graph_feature], dim=-1)

        out = self.aft(fusion_input)

        out = torch.cat([out, fusion_input], dim=-1)
        ...
        # 特征对齐
        dist = [text_feature, graph_feature, image_feature]

        features = torch.concat([out.unsqueeze(1), out.unsqueeze(1)], dim=1)

        # MLP
        a1 = self.relu(self.dropout(self.fc1(out)))
        a1 = self.relu(self.fc2(a1))
        a1 = self.relu(self.fc3(a1))
        a1 = self.dropout(a1)
        output = self.fc4(a1)
        return F.sigmoid(output), dist, features
