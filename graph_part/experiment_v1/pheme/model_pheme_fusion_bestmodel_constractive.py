...
class NeuralNetwork(nn.Module):
    ...
    def mfan(self, x_tid, x_text, y, loss, i, total, params, pgd_word):
        ...
        # 此时hidden的shape为[batch_size,2,output_dim],为每一份数据创建了一个副本，进行对比
        loss_cons = SupConLoss()
        loss_constrative = loss_cons(features, y)

        loss_defense = 1.8 * loss_classification + 2.4 * loss_dis + 0.12*loss_constrative
