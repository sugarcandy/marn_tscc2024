...
class NeuralNetwork(nn.Module):
    ...
    def mfan(self, x_tid, x_text, y, loss, i, total, params, pgd_word):
        ...
        loss_cons = SupConLoss(temperature=0.1)
...
def train_and_test(model):
    ...
    config['save_path'] = os.path.join(res_dir, args.thread_name + '_' + 'config' + args.config_name.split(".")[
        0] + '_best_model_weights_' + model_suffix + 'lstmwithclip_nllLoss_temp0.1')
    ...
    nn.load_state_dict(torch.load(
        "exp_result/weibo2/exp_description/best_model_in_each_config/Thread-1_configsingle3_best_model_weights_mfanlstmwithclip_nllLoss_temp0.1"))
